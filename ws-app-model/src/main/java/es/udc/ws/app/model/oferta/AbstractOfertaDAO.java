...
public abstract class AbstractOfertaDAO implements OfertaDAO {
	...
	public Oferta find(Connection connection, Long ofertaId)
		...
		try (PreparedStatement preparedStatement = connection
			...
			/* Return oferta. */
			return new Oferta(ofertaId, nombreOferta, descripcionOferta,
					estadoOferta, precioRealOferta, precioDescontadoOferta,
					comisionOferta, fechaLimiteOferta, fechaLimiteReserva,
					facebookId);
		...
		try (PreparedStatement preparedStatement = connection
			...
			if (words != null) {
				...
				for (int i = 0; i < words.length; i++) {
					...
					System.out.println("preparedstmt: " + preparedStatement);
				}
			}
			System.out.println("preparedstmt: " + preparedStatement);
			if (estadoBusqueda != null)
				preparedStatement.setString(j++, estadoBusqueda);
			Timestamp date = fechaBusqueda != null ? new Timestamp(
					fechaBusqueda.getTime().getTime()) : null;
			if (fechaBusqueda != null)
				preparedStatement.setTimestamp(j++, date);
			...
			while (resultSet.next()) {
				...
				ofertas.add(new Oferta(ofertaId, nombreOferta,
						descripcionOferta, estadoOferta, precioRealOferta,
						precioDescontadoOferta, comisionOferta,
						fechaLimiteOferta, fechaLimiteReserva, facebookId));
